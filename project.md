...
10. 新增 `vision_rgb565_color_top` 独立彩色实验顶层，使用已验证稳定的 YUV422 摄像头配置，在彩色顶层内转换为 RGB565 并写入 16-bit RGB565 彩色帧缓存，不再复用灰度/Sobel 处理链路。
11. Sobel/漫画显示已优化：
...
`SW2/M4 style_page_sw` 在主顶层中不再切换彩色页。彩色实验需要把 Vivado top 改为 `vision_rgb565_color_top`。该顶层只服务彩色调试和风格化，不再包含灰度图处理页：
