...
rtl/vision_rgb565_color_top.v:
  ...
  SW2=1 彩色识别/风格页：
    00 = RGB565 直接彩色
    01 = RGB565 高低字节交换后彩色，用于判断真实字节序
    10 = 彩色像素漫画风，4x4 像素块采样 + 色彩分层 + Sobel 暗轮廓
    11 = 二次元色块风，色彩分层 + Sobel 暗轮廓
  若 01 的颜色顺序正确，把 `RGB565_BYTE_SWAP` 设为 `1'b1`，让 10/11 也使用交换后的字节序
...
SW2 = 1: 彩色扩展页
00: 彩色原图
01: 高低字节交换后的彩色原图，用于判断 RGB565 字节序
10: 彩色像素漫画风，4x4 像素块采样 + 色彩分层 + Sobel 暗轮廓
11: 二次元色块风，色彩分层 + Sobel 暗轮廓
```
...
如果 `SW2=1, mode=01` 的颜色明显比 `mode=00` 正确，说明真实模块需要 RGB565 高低字节交换。此时把 `rtl/vision_rgb565_color_top.v` 中的 `RGB565_BYTE_SWAP` 改成 `1'b1`，彩色像素漫画和二次元模式会跟随使用交换后的字节序。
