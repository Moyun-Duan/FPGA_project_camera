...
rtl/vision_rgb565_color_top.v:
  ...
  YUV_TO_RGB = 1
  ov7670_init RGB565_CONFIG = 0
  复用已验证稳定的 OV7670 YUV422 输出格式，在彩色顶层内部转换为 RGB565
  使用 16-bit RGB565 彩色帧缓存显示转换后的彩色数据
  不再实例化 pixel_filter_pipeline，也不再使用灰度帧缓存
  SW2=0 彩色通道检查页：
    00 = 标准 UYVY 转 RGB，带 1.5 倍色度增强
    01 = U/V 色度字节互换
    10 = 红/绿输出通道互换
    11 = V 色度方向反转
  SW2=1 彩色风格页：
...
彩色实验顶层现在只保留彩色路径。它把 OV7670 配置为已经在 `vision_top` 中验证过的 YUV422 模式，在 `vision_rgb565_color_top` 内部自行把 U/Y/V/Y 数据转换为 RGB565，并把转换后的 16-bit RGB565 写入彩色缓存。转换时默认把 U/V 色度放大 1.5 倍，避免画面过度接近黑白。

如果 SW2=0 的某个调试模式明显更自然，按结果修改 `rtl/vision_rgb565_color_top.v` 顶部参数：mode 01 对应 `COLOR_UV_SWAP=1'b1`，mode 10 对应 `COLOR_SWAP_RG=1'b1`，mode 11 对应 `COLOR_INVERT_V=1'b1`。如果后续发现红蓝反了，再把 `COLOR_SWAP_RB` 改成 `1'b1`。修改参数后重新综合/实现，再测试 SW2=1 的风格页。
