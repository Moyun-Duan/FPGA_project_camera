...
`rtl/vision_top.v` 中摄像头 `cam_xclk` 已改为由 SW4 选择：

```text
pix_clk  = clk_100m / 4  = 25 MHz，用于 VGA
SW4 = 0: cam_xclk = clk_100m / 8  = 12.5 MHz
SW4 = 1: cam_xclk = clk_100m / 4  = 25 MHz
```

目的：

```text
12.5 MHz: 降低 OV7670 PCLK 和并行数据翻转速度，提高杜邦线连接下的数据采样余量
25 MHz: 测试更高帧率是否能改善拖影、自动曝光稳定性或画面刷新
```

注意：

```text
提高 XCLK 不会提高 320x240 的空间分辨率。
如果 25 MHz 下噪声增加或花屏，说明采样/布线余量不足，应回到 12.5 MHz。
切换 SW4 后按一次 RESET，让 OV7670 在稳定 XCLK 下重新初始化。
```
...
constraints/ego1_template.xdc:
  camera_yuv_order_sw = SW2/M4
  image_enhance_sw = SW3/R2
  camera_fast_xclk_sw = SW4/P2
```

当前工程已经支持运行时切换 YUV 取样相位：

```text
SW2 = 0: 按 YUYV 取 Y
SW2 = 1: 按 UYVY 取 Y，屏幕右上角出现黄色块
```

调试步骤：

```text
1. 固定 mode_sw=00，也就是蓝色边框原始亮度图。
2. 拨动 SW2。
3. 观察哪一档人物和桌面轮廓更自然、更清晰。
4. 后续测试 Sobel/素描时保持更清晰的 SW2 位置。
```

如果 SW2 两档都很模糊，说明主要问题不是 YUV 字节顺序，而是采样/布线/时序或 OV7670 寄存器配置。
...
```text
SW3 = 0: 标准处理
SW3 = 1: 清晰度增强，屏幕右上角出现紫色块
SW4 = 1: 25 MHz XCLK，屏幕右上角出现青色块
```

该增强不是提高真实分辨率，而是在 `mode_sw=01` 时把原来的平滑灰度改为 3x3 反锐化灰度：

```text
00: 原始灰度
01, SW3=0: 3x3 平滑灰度，噪声更少但会变软
01, SW3=1: 3x3 锐化灰度，边缘和文字轮廓更明显
10: Sobel 二值边缘
11: 素描风格
```
...
```text
SW2 = 1, mode_sw = 00: 原始灰度，作为判断采集质量的基准
SW2 = 1, mode_sw = 01, SW3 = 0: 平滑灰度，适合看大轮廓
SW2 = 1, mode_sw = 01, SW3 = 1: 锐化灰度，适合看文字、边缘、人物轮廓
```

如果 `SW3=1` 后噪点也被增强，说明当前画面主要受采样噪声或光照影响，应回到 `SW3=0`。
...
```text
1. 确认 YUV 相位正确。
2. 比较 SW4=0/1，判断 12.5 MHz 与 25 MHz 哪个更稳定。
3. 改善布线和 PCLK 采样稳定性。
...
5. 用 SW4 对比 12.5 MHz 和 25 MHz：25 MHz 更清楚才保留，否则用 12.5 MHz。
