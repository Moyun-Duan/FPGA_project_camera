...
- 模式选择：`mode_sw[1:0]`
  ...
  - `10`：白底黑线草图。
  - `11`：五级灰度漫画化 + 黑色轮廓线。
- 彩色实验已拆到 `rtl/vision_rgb565_color_top.v`，该顶层使用已验证稳定的 OV7670 YUV422 配置，在彩色顶层内转换为 RGB565 并写入独立 16-bit RGB565 彩色帧缓存，不再复用灰度/Sobel 处理链路；`SW2=0` 做 U/V、红绿通道和 V 方向检查，`SW2=1` 做彩色原图、海报化、像素风和二次元色块风。
