...
| `style_page_sw` | 输入 | `M4` | `LVCMOS33` | 拨码开关 SW2 | 保留端口兼容性，最终灰度顶层不使用 |
...
完整工程 `vision_top` 中，`style_page_sw` 仅保留端口兼容性，不参与模式选择。`mode_sw[1:0]` 含义为：
...
mode 01 = 锐化灰度
cam_xclk = 25 MHz
SW2/style_page_sw 当前只保留接口兼容，不再作为 YUV 相位或彩色页调试开关
```
