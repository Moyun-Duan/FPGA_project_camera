...
| `style_page_sw` | 输入 | `M4` | `LVCMOS33` | 拨码开关 SW2 | 完整工程中选择原功能页/彩色扩展页 |
...
| `cam_xclk` | 输出 | `B14` | `J5-12` | `LVCMOS33` | `XCLK` | 固定约 25 MHz；先测这个信号 |
...
完整工程 `vision_top` 中，`style_page_sw` 和 `mode_sw[1:0]` 含义为：

`style_page_sw = 0` 时保持原有功能页：

| `mode_sw[1:0]` | 模式 | 说明 |
|---|---|---|
| `00` | 灰度图 | 首次摄像头调试优先使用此模式 |
| `01` | 锐化灰度图 | 3x3 反锐化，增强轮廓 |
| `10` | 白底黑线草图 | 强边缘黑线、弱边缘浅灰线 |
| `11` | 漫画化图像 | 五级灰度色块 + 黑色轮廓线 |

`style_page_sw = 1` 时启用彩色扩展页：

| `mode_sw[1:0]` | 模式 | 说明 |
|---|---|---|
| `00` | 彩色原图 | YUV422 转 RGB 后显示 |
| `01` | 彩色像素漫画风 | 色彩分层 + 像素块采样 + 黑色轮廓 |
| `10` | 白底黑线草图 | 与原功能页一致 |
| `11` | 漫画化图像 | 与原功能页一致 |

摄像头首次上板时，不建议先看 Sobel。先用 `00` 灰度图确认采集链路，再切换其它模式。

当前完整工程已经移除 SW3/SW4 调试输入，并固定为实测较优配置：

```text
YUV 取样相位 = 原 SW2=1
mode 01 = 锐化灰度
cam_xclk = 25 MHz
SW2 当前重新作为 style_page_sw 使用，不再作为 YUV 相位调试开关
```
...
|---:|---|---|---|
| 1 | `cam_xclk` | 约 25 MHz | `clk_100m`、`reset_n`、`B14/J5-12` 接线 |
...
```text
1. mode_sw = 00，看灰度图
2. 如果有灰度图但亮度异常，检查 YUV 取样相位和 D0-D7 顺序
3. 灰度图稳定后，切 mode_sw = 10 看白底黑线草图
4. Sobel 过黑或过白，再调整 Sobel 低/高阈值
5. 最后测试 mode_sw = 11 漫画化模式
```
